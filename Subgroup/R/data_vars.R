...
analysis_data <- S7::new_class("analysis_data",
                               properties = list(
                                   ...
                                   data_t0 = S7::new_property(S7::class_data.frame),
                                   data_t1 = S7::new_property(S7::class_data.frame)
                               ),
                               constructor = function(data, varn, tt0, tt1) {
                                   ...
                                   S7::new_object(
                                       ...
                                       data_t0 = shift_data_t(data, tt = varn@tt, t_shift = tt0),
                                       data_t1 = shift_data_t(data, tt = varn@tt, t_shift = tt1)
                                   )
                               },
                               validator = function(self) {
...
S7::method(training, analysis_data) <- function(x, fold_obj, fold) {
    list(
        ...
        data_t0 = x@data_t0[fold_obj[[fold]]$training_set, , drop = FALSE],
        data_t1 = x@data_t1[fold_obj[[fold]]$training_set, , drop = FALSE]
    )
}
...
S7::method(validation, analysis_data) <- function(x, fold_obj, fold) {
    list(
        ...
        data_t0 = x@data_t0[fold_obj[[fold]]$validation_set, , drop = FALSE],
        data_t1 = x@data_t1[fold_obj[[fold]]$validation_set, , drop = FALSE]
    )
}
